 ...
 * @version $Id: view_render_php.php 2629 2009-07-17 10:28:03Z firzen $
 * @package mvc
 */
...
class View
{
    ...
    function fetch($viewname, array $vars = null, array $config = null)
    {
        ...
        if (file_exists($filename))
        {
            ...
            {
                $this->_parser = new View_Parser($view_dir,$this->view_dir_base);
            }
...
class View_Parser
{
     ...
     */
    private $_view_dir;
    private $_view_dir_versionbase;     
    private $_view_dir_base;
    ...
    function parse($filename, $view_id = null, array $inherited_stack = null)
    {
        ...
        if ($stack['extends'])
        {
            ...
            if($this->_view_dir_versionbase && !file_exists($filename)){
            	$filename = "{$this->_view_dir_versionbase}/{$stack['extends']}.{$this->_extname}";
            }
            if(!file_exists($filename)){
                $filename = "{$this->_view_dir_base}/{$stack['extends']}.{$this->_extname}";
            }
