...
class QGenerator_Application
{
	...
	function generate($appid, $dir, $tpl = null, $skeleton_dir = null)
	{
		...
		{
		    throw new QGenerator_Exception(Q::__('Specified tpl "%s" notexistent.', $tpl));
		}

		$appid = strtolower(preg_replace('/[^a-z0-9_]+/i', '', $appid));
		if (empty($appid))
		{
			throw new QGenerator_Exception(Q::__('Invalid appname "%s".', $appid));
		}

		if (!is_dir($dir))
		{
			throw new QGenerator_Exception(Q::__('Invalid dir "%s".', $dir));
		}
		...
		if (is_dir($appdir))
		{
		    throw new QGenerator_Exception(Q::__('Application dir "%s" already exists.', $appdir));
		}

		if (!mkdir($appdir, 0777))
		{
			throw new QGenerator_Exception(Q::__('Creation application dir "%s" failed.', $appdir));
		}
