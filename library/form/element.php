<?php
// $Id: element.php 2401 2009-04-07 03:50:08Z dualface $
 ...
 * @version $Id: element.php 2401 2009-04-07 03:50:08Z dualface $
 * @package form
 */
...
class QForm_Element
{
    ...
    function __construct($id = null, array $attrs = null, QForm_Group $owner = null)
    {
        ...
        if ($owner)
        {
            ...
            {
                $this->_nested_name = $owner->nestedName();
            }
        }
    }
    ...
    function add($type, $id, array $attrs = null)
    {
        ...
        throw new QForm_Exception(
            Q::__('当前元素 "%s" 不属于任何群组，因此无法完成 add() 操作.', $this->id)
        );
    }
     ...
     * @return QForm_Element
     */
    function import($data,&$parent_data =array())
    {
        if ($this->_data_binding && !$this->_locked)
        {
        	/*
        	 * @firzen
        	 * 增加外部import 调用
	        ...
	        if (method_exists('Control_'.$this->get('_ui'),'__import')){
	    		$this->_attrs['value'] = call_user_func_array(array('Control_'.$this->get('_ui'),'__import'),array($this,$data,&$parent_data));
	    	}else {
    ...
    function validate($data, & $failed = null)
    {
        ...
        $_sexy=method_exists('Control_'.$this->get('_ui'),'__validate');
        if (!empty($this->_validations) || $_sexy)
        {
            $failed = null;
            /*
             * @firzen
             * 增加对外部调用validate 支持
             * $this,$data,$failed,$_is_valid
             */
            $_is_valid=(bool)QValidator::validateBatch($data, $this->_validations, QValidator::CHECK_ALL, $failed);
            $this->_is_valid = $_is_valid;
            if ($_sexy){
        	    $this->_is_valid=call_user_func_array(array('Control_'.$this->get('_ui'),'__validate') ,array($this ,$data ,&$failed, $_is_valid));
            }
    ...
    function value()
    {
    	 ...
    	 * $this
    	 */
    	if (method_exists('Control_'.$this->get('_ui'),'__value')){
    		return call_user_func(array('Control_'.$this->get('_ui'),'__value') ,$this );
    	}
        return $this->value;
    }
