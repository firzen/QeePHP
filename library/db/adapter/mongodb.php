...
// $Id: mongodb.php  2014-02-21 Tim $

/**
 * 定义 QDB_Adapter_MongoDB 类
 *
 * @link http://qee13.com/
 * @copyright Copyright (c) 2014-2014 Jutian Inc. {@link http://www.qee13.com}
 * @license New BSD License
 * @version $Id: mongodb.php  2014-02-21 Tim $
 * @package database
 */

/**
 * QDB_Adapter_MongoDB 提供了对 mongodb 数据库的支持
 *
 * @author Tim Pan <tim@qee13.com>
 * @version $Id: mongodb.php  2014-02-21 Tim $
 * @package database
 */
class QDB_Adapter_Mongodb extends QDB_Adapter_Abstract
{
    ...
    function connect(){
    	...
    	try {
    		$this->_conn = new MongoClient("mongodb://".$this->_dsn['host'],array('username'=>$this->_dsn['login'],'password'=>$this->_dsn['password']));
    	}catch (Exception $ex){
    		throw new QDB_Exception('CONNECT DATABASE',$ex->getMessage(), $ex->getCode());
    	}
    	
    	if (!$this->_conn->connected)
    	{
    		throw new QDB_Exception('CONNECT DATABASE','FAILURE');
    	}
    	
    	if (! empty($this->_dsn['database']))
    	{
//     		$this->_conn->selectDB($this->qid($this->_dsn['database']));
    	}
    ...
    function qinto($sql, array $params = null, $param_style = null, $return_parameters_count = false)
    {
        ...
        case QDB::PARAM_AT_NAMED:
            ...
            for ($offset = 1; $offset < $max; $offset += 2)
            {
                ...
                {
                    throw new QDB_Exception($sql, Q::__('Invalid parameter "%s" for "%s"', $arg_name, $sql), 0);
                }
    ...
    function update($table_name, $row, array $where = null, array $restricted_fields = null)
    {
        ...
        {
            throw new QDB_Exception(Q::__('Type mismatch'));
        }
    ...
    function metaColumns($table_name){
    	...
    	$collection=$this->_conn->selectCollection($this->_dsn['database'], '_qmeta');
    	$doc=$collection->findOne(array('name'=>$table_name));
    	echo $table_name;
    	require_once dirname(__FILE__). '/mongodb/PHPSQLParser.php';
    	$parser = new PHPSQLParser(" SELECT `base_ebay_user`.*   FROM `tl`.`base_ebay_user`   WHERE (a=b and (uid ='2' and state1=1)) and x=y", true);
    	dump($parser->parsed,1,11);
    	
    	$rs = $this->execute(array('name'=>$table_name),array($table_name));
