...
class QDB_Select
{
    ...
    const PAGED_QUERY_COUNT = 'paged_query_count';
     ...
     * @return QDB_Select
     */
    function distinct($flag = true,$cols=array())
    {
//    	if (empty($cols) || (bool)$flag ==false){
	        $this->_parts[self::DISTINCT] = (bool) $flag;
//    	}else {
//    		$this->_parts[self::DISTINCT] = Q::normalize($cols);
//    	}
    	return $this;
    }
    ...
    function link($link)
    {
        ...
        foreach ($links as $link)
        {
            ...
            else
            {
                ...
                throw new QDB_Select_Exception(Q::__('关联必须是 QDB_ActiveRecord_Association_Abstract 类型.'));
            }
        }
    ...
    function union($select = array(), $type = self::SQL_UNION)
    {
        ...
        if (! isset(self::$_union_types[$type]))
        {
            ...
            throw new QDB_Select_Exception(Q::__('无效的 UNION 类型 "%s".', $type));
        }
    ...
    function getPagination()
    {
        ...
        if ($this->_parts[self::GROUP]){
        	...
        	unset($this->_parts[self::GROUP]);
        	$this->_parts[self::PAGED_QUERY_COUNT]=reset($group);
        }
    ...
    function getPart($part)
    {
        ...
        {
            // LC_MSG: 无效的部分名称 "%s".
            throw new QDB_Select_Exception(Q::__('无效的部分名称 "%s".', $part));
        }
    ...
    function getById($id,$included_links = null){
    	if ($this->_meta->idname_count !=1){
    		throw new QDB_Select_Exception(Q::__('getById 方法只适用于单一主键模型'));
    	}
        ...
        if (strncasecmp($method,'get',3) === 0)
        {
            ...
            // support getByName getByNameAndSex etc.
            	...
            	if (count($keys) != count($args)){
            		throw new QDB_Select_Exception(Q::__('参数数量不对应'));
            	}
    ...
    protected function _renderColumns()
    {
        ...
        {
        	if (isset($this->_parts[self::PAGED_QUERY_COUNT])){
        		return 'COUNT(DISTINCT('.$this->_parts[self::PAGED_QUERY_COUNT] .' ))';
        	}
            return 'COUNT(*)';
        }
    ...
    protected function _join($join_type, $name, $cols, $cond = null, $cond_args = null)
    {
        if (! isset(self::$_join_types[$join_type]))
        {
            ...
            throw new QDB_Select_Exception(Q::__('无效的 JOIN 类型 "%s".', $join_type));
        }

        if (count($this->_parts[self::UNION]))
        {
            // LC_MSG: 不能在使用 UNION 查询的同时使用 JOIN 查询.
            throw new QDB_Select_Exception(Q::__('不能在使用 UNION 查询的同时使用 JOIN 查询.'));
        }
