    ...
    function flush()
    {
        ...
        if ($dir === false || empty($dir))
        {
            ...
            if ($dir === false || empty($dir))
            {
                ...
                {
                    throw new QLog_Exception(Q::__('指定的日志文件保存目录不存在 "%s".', Q::ini('log_writer_dir')));
                }
            }
        }
        ...
        // 大于25M 分割文件
        if (@filesize($this->_filename)/1024/1024 >25){
        	rename($this->_filename,$this->_filename.'.'.date('His'));
        }
