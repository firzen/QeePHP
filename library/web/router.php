...
class QRouter
{
    ...
    protected function _prepareRegexRoute($route_name, array $rule)
    {
        ...
        if (preg_match($route['regex'], '') === false)
        {
            ...
            throw new QRouter_InvalidRouteException($route_name, Q::__($msg, $route_name, $rule['regex']));
        }
        ...
        if (count($m[1]) != count($rule['config']))
        {
            ...
            throw new QRouter_InvalidRouteException($route_name, Q::__($msg, $route_name, count($m[1]), count($rule['config'])));
        }
        ...
        foreach ($parts as $pos => $part)
        {
            if ($part{0} == $this->_var_prefix)
            {
                ...
                if ($use_static_optional && !isset($rule['defaults'][$varname]))
                {
                    ...
                    throw new QRouter_InvalidRouteException($route_name, Q::__($msg, $route_name, $varname));
                }
            ...
            else
            {
                ...
                else
                {
                    if ($use_static_optional)
                    {
                        ...
                        throw new QRouter_InvalidRouteException($route_name, Q::__($msg, $route_name, $part));
                    }
